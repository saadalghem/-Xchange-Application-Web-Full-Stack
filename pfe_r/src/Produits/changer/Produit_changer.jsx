...
function Produit_changer() {
    ...
    useEffect(() => {
        axios.get("http://localhost:8081/api/produit/changer")
            .then((res) => {
        ...
        <div className="min-h-screen bg-gradient-to-br from-purple-100 to-blue-100 py-12 px-6">
            <h2 className="text-4xl font-extrabold text-center mb-10 text-purple-800 drop-shadow-lg">
                {selectedCategorie || selectedLocation ?
                    `Produits à changer ${selectedCategorie ? `- ${categories.find(cat => cat.value === selectedCategorie)?.label}` : ''} 
                ...
                {filteredProduits.map((prod) => (
                    <Link to={`/produit/${prod.id}`} key={prod.id}
                        ...
                        <div className="mt-auto">
                            ...
                            <span className="inline-block px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full mt-2 w-full text-center">
                                À echanger
                            </span>
                        </div>
                    </Link>
...
export default Produit_changer;